...
$(function(){
    ...
    /*底部导航跳转*/
    let phone = parseInt($.getUrlParam("ph"));
            ...
            success:function(res){
                if((token!=null&&token!="")&&res.code==2000){
                    if(i==2){
                        ...
                        let url = my_pintuan+".html?ph="+phone;
                        jumpToDetailView(url);
                    }else if(i==3){
                        //location.href = my+".html?ph="+phone;
                        let url = my+".html?ph="+phone;
                        jumpToDetailView(url);
                    }
                }else{
                    //location.href = login_one+".html?ym_id="+0+"&foot_id="+foot_id;
                    let url = login_one+".html?ym_id="+0+"&foot_id="+foot_id;
                    jumpToDetailView(url);
                }
            },
    ...
    $(".ui-footer ul li").on("click",function(){
        ...
        switch (i){
            case 0:
                ...
                let url = index+".html?ph="+phone;
                jumpToDetailView(url);

                break;
            case 1:
                //location.href = choose_car+".html?ph="+phone;
                let url_ = choose_car+".html?ph="+phone;
        ...
        $.ajax({
            ...
            $.ajax({
                ...
                success:function(res){
                    //console.log(res);
                    let len = res.data.length;
                    let li = '<li class="ui-col  ui-col-50"><a href="javascript:void(0);"><img class="car_new_img" src="img/images/new_car.png"/><div class="car_new_txt"><div>福特斯瑞斯 2017款 1.5L自动舒适型</div><span>团购价<span>2.5</span>万</span><span>优惠<span>2.5</span>万</span></div></a></li>'
                    $("#xpsj li").remove();
                    for(let i=0; i<len;i++){
                        let cx = res.data[i].model_name,
                            pb = res.data[i].name,
                            cl = "";
                        if(cx.indexOf(pb)!=-1){
                            cl = cx;
                        ...
                        let w = $("#xpsj .car_new_img").width();
                        let h = w/1.7;
                        //console.log(h);
                        $("#xpsj .car_new_img").css("height",h);

                        //let st = $.getUrlParam("st_h");
